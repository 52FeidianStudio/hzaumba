...
class IndexController extends Controller {
    public function index(){
		$home=M('home');
	    $hq_a['class']="新闻中心";
	    $info_new=$home->order('hid desc')->where($hq_a)->limit(8)->select();
	    //dump($info);
        $this->assign('info_new',$info_new);
		
		 $hq_b['class']="通知公告";
	    $info_tzgg=$home->order('hid desc')->where($hq_b)->limit(8)->select();
	    //dump($info);
        $this->assign('info_tzgg',$info_tzgg);
		
		 $hq_c['class']="招生信息";
	    $info_zsxx=$home->order('hid desc')->where($hq_c)->limit(8)->select();
	    //dump($info);
        $this->assign('info_zsxx',$info_zsxx);
		
		 $hq_d['class']="教务信息";
	    $info_jwxx=$home->order('hid desc')->where($hq_d)->limit(8)->select();
	    //dump($info);
